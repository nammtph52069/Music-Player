...
function MediaItems({ isActive }) {
  ...
  // const [widthCardInforbRef, setWidthCardInforbRef] = useState(0)
  const itemRef = useRef(null)
  const songThumbRef = useRef(null)
  const cardInforbRef = useRef(null)
  const hoverRef = useRef(null)

  useEffect(() => {
    if (isActive && itemRef.current && songThumbRef.current && cardInforbRef.current) {
      // const widthItemRef = itemRef.current.offsetWidth
      // const widthSongThumbRef = songThumbRef.current.offsetWidth
      // const widthCardInforbRef = cardInforbRef.current.offsetWidth
      // const widthHoverRef = hoverRef.current.offsetWidth
      // setWidthCardInforbRef(widthCardInforbRef)
    }
  }, [isActive])

  return (
    <div ref={itemRef} className={`p-2 rounded group/items ${isActive ? 'bg-[#9b4de0]' : ' hover:bg-[#ffffff1a]'} text-white`}>
      <div className="flex items-center">
        <div className="w-full flex items-center gap-2">
          <div ref={songThumbRef} className="songThumb cursor-pointer relative">
          ...
          <div ref={cardInforbRef} className="cardInfor w-[166px] flex flex-col flex-shrink flex-grow basis-0">
            <div className="">
              <div className="title">
          ...
          <div ref={hoverRef} className="hidden group-hover/items:block">
